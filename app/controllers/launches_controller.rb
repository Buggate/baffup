...
def create
  ...
  emails = params[:launch][:email].split(',')
   ...
   if  @launch.save
     ...
     LaunchMailer.baffsup_occasion(@launch, @operator, new_user_registration_url(:email => @launch.email, :password => @placeholder, :password_confirmation => @placeholder )).deliver_now     
